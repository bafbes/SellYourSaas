...
function sellyoursaas_completesubstitutionarray(&$substitutionarray, $langs, $object, $parameters=null)
{
	...
	if ((! empty($parameters['mode'])) && $parameters['mode'] == 'formemail') {	// For exemple when called by FormMail::getAvailableSubstitKey()
		...
		if (is_object($object) && get_class($object) == 'Contrat') {
            ...
            $lang=new Translate('',$conf);
			$lang->setDefaultLang($object->thirdparty->default_lang=='fr_FR'?'fr_FR':'en_US');
            $lang->load('main');
            $substitutionarray['__SELLYOURSAAS_EXPIRY_DATE__'] = dol_print_date($object->array_options['options_date_endfreeperiod'],'',  'auto',$lang);
        }
    }
	...
	//Si la constante SELLYOURSAAS_USE_THIRDPARTY_DOMAIN_REGISTRATION_PAGE, on n'enverra des mails qu'avec l'adresse mail d'envoi utilisée lors de l'enregistrement
	if (! empty($conf->global->SELLYOURSAAS_USE_THIRDPARTY_DOMAIN_REGISTRATION_PAGE) &&is_object($tmpobject) && ! empty($tmpobject->array_options['options_domain_registration_page'])) {
		global $savconf;
